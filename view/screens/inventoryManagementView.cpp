...
InventoryManagementView::InventoryManagementView() {
    ...
    setMenuActions({
        [this]() { 
            // 재고 조회 화면으로 이동
            goToScreen(std::make_shared<InventoryListView>());
        },
        [this]() { 
            // 재고 입력 화면으로 이동
            goToScreen(std::make_shared<InventoryAddView>());
        },
        [this]() { 
            // 재고 증가/차감 화면으로 이동
            goToScreen(std::make_shared<InventoryModifyView>());
        }
    });
}

int InventoryManagementView::getUserChoice() {
    int choice;
...
void InventoryManagementView::run() {
    while (true) {
        // 화면 지우기
        clearScreen();
